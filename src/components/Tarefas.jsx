...
export function Tarefas() {
  ...
  const fetchTarefas = async () => {
    try {
      ...
      const { data, error } = await supabase
        .from('tarefas')
        .select('*')
        .order('criado_em', { ascending: false }); // Corrigido: Ordena pela data de criação

      if (error) throw error;

      // Traduz o 'status' do banco para o booleano 'isCompleted' da interface
      const tarefasFormatadas = (data || []).map(t => ({
        id: t.id,
        titulo: t.titulo,
        prioridade: t.prioridade,
        isCompleted: t.status === 'concluid', // Compatibilidade inline
  ...
  const toggleTarefa = async (tarefa) => {
    const novoStatus = !tarefa.isCompleted;
    const novoStatusTexto = novoStatus ? 'concluid' : 'pendente';
    
    setTarefas(tarefas.map(t => 
      t.id === tarefa.id ? { ...t, isCompleted: novoStatus } : t
    ));
    ...
    try {
      const { error } = await supabase
        ...
        .update({ 
          ...
          concluida_em: novoStatus ? new Date().toISOString() : null // Preenche o timestamp da sua tabela
        })
        .eq('id', tarefa.id);
      ...
      setTarefas(tarefas.map(t => 
        t.id === tarefa.id ? { ...t, isCompleted: tarefa.isCompleted } : t
      ));
    }
  };
  ...
  const reiniciarTarefas = async () => {
    const confirmar = window.confirm("Deseja marcar todas as tarefas concluídas como pendentes?");
    if (!confirmar) return;
  ...
  const getBadgeColors = (prioridade, isCompleted) => {
    ...
    switch (prioridade) {
      ...
      case 'media': return 'bg-purple-50 text-purple-600 dark:bg-purple-900/20 dark:text-purple-400';
  ...
  return (
    <div className="flex flex-col gap-8 max-w-4xl mx-auto w-full pb-10">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          ...
          <button 
            ...
            className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-arrumei-purple hover:bg-arrumei-purple-light text-white font-semibold text-sm shadow-md transition-all"
          >
      ...
      <div className="flex flex-col gap-3">
        {loading ? (
          <div className="text-center py-10 text-gray-400">Carregando tarefas...</div>
        ) : (
          tarefasFiltradas.map((tarefa) => (
            ...
            >
              ...
              <div className="flex items-center gap-4 flex-1 cursor-pointer" onClick={() => toggleTarefa(tarefa)}>
